  ...
  <div class="appointment-list">
    <div class="appointment" v-for="(appointment,i) in listData" :key="i">
      ...
      <div class="appointment-conetnt">
        <div class="time">预约时间：{{appointment.createTime}}</div>
        <div class="user">联系人：{{appointment.linkUser}}</div>
        <div class="detail">补充说明：{{appointment.detail}}</div>
      </div>
      <div class="appointment-footer">
        <div class="btn" @click="$router.push({name:'addappointment',params:{appointment}})">查看详情</div>
        <div class="btn" v-if="appointment.status" @click="$router.push({name:'addappointment',params:{appointment}})">修改预约</div>
